...
class FakeSpeechToText(Recorder):
    def procesar(self, playlist_url: str) -> list[ActorTranscript]:
        self.calls.append(f"{self.name}:{playlist_url}")
        return [_actor_transcript()]


class FailingSpeechToText(Recorder):
    def procesar(self, playlist_url: str) -> list[ActorTranscript]:
        self.calls.append(f"{self.name}:{playlist_url}")
...
class GranularSpeechToText:
    ...
    def discover(self, playlist_url: str) -> tuple[PlaylistInfo, list[VideoMeta]]:
        ...
        return PlaylistInfo(nombre="playlist", url=playlist_url), [
...
def _factories(calls: list[str], *, failing_shape: bool = False) -> ExecutionFactories:
    return ExecutionFactories(
        build_speech2text=lambda _cfg: FakeSpeechToText("speech2text", calls),
...
def _factories_with_failing_speech(calls: list[str]) -> ExecutionFactories:
    return ExecutionFactories(
        build_speech2text=lambda _cfg: FailingSpeechToText("speech2text", calls),
...
""",
    ...
    assert calls == ["speech2text:playlist-url", "argument_shape:1"]
...
""",
    ...
    assert result.exit_code == 1
    assert calls == ["speech2text:playlist-url"]
